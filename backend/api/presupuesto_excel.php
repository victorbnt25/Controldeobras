...
$fecha = $presupuesto['fecha_presupuesto'] ?? date('d/m/Y');
...
$hoja->setCellValue('E1', 'PRESUPUESTO');
...
$hoja->setCellValue('G1', 'PRESUPUESTO');
...
$hoja->setCellValue('E'.$filaC, 'Obra:  ' . ($presupuesto['nombre_obra'] ?? ''));
...
// Calcular totales
$ivaPorcentaje = floatval($presupuesto['iva_porcentaje'] ?? 21); 
...
if ($formato === 'pdf') {
    ...
    if (strpos($htmlBody, '</head>') === false) {
        $htmlBody = $cssExtra . $htmlBody;
    }

    // Footer de 2 filas ≈ 22mm
    $footerH = 22;
    $mpdf = new \Mpdf\Mpdf([
        'format' => 'A4',
    ...
    // Div espaciador: solo bordes LATERALES para continuar el rectángulo
    if ($gap > 0) {
    ...
    header('Content-Disposition: attachment;filename="Presupuesto_' . $nombreArchivoNum . '.pdf"');
    ...
    else { $hoja->setCellValue('C'.$filaActual, 0); }
    $hoja->getStyle('C'.$filaActual)->getNumberFormat()->setFormatCode('#,##0.00 €');
    $hoja->setCellValue('D'.$filaActual, "IVA: " . number_format($ivaPorcentaje, 2) . "%");
    $hoja->setCellValue('E'.$filaActual, "=C$filaActual*" . ($ivaPorcentaje/100));
    $hoja->getStyle('E'.$filaActual)->getNumberFormat()->setFormatCode('#,##0.00 €');
    $hoja->setCellValue('F'.$filaActual, 'Total');
    $hoja->setCellValue('G'.$filaActual, "=C$filaActual+E$filaActual");
    ...
    $hoja->getStyle('D'.$filaActual)->applyFromArray($fondoGris);
    ...
    $nombreArchivoNum = $numero !== '000/00' ? str_replace('/', '-', $numero) : 'Borrador';
    header('Content-Disposition: attachment;filename="Presupuesto_' . $nombreArchivoNum . '.xlsx"');
