...
export default function FacturaForm({ factura, onSaved, onCancel }) {
  ...
  const cargarDatosEdicion = async () => {
    try {
      ...
      if (!factura.items) {
        data = await api.obtenerFactura(factura.id);
      }

      setNumero(data.numero_factura || "");
      setFecha(data.fecha_factura || "");
      setIdCliente(data.cliente_id || "");
      setDescripcion(data.oficio || "");
      setIva(Number(data.iva_porcentaje) || 0);
      setEstado(data.estado || "pendiente");
      setFormaPago(data.forma_pago || "Transferencia bancaria");
  ...
  const alGuardarFactura = async () => {
    ...
    if (!descripcion.trim()) {
      setError("El nombre/descripción de la factura es obligatorio.");
      return;
    }

    if (!numero.trim()) {
      setError("El número de factura es obligatorio (ej: 001/26).");
      return;
    }
    ...
    try {
      ...
      let respuesta;
      if (factura) {
        cargaUtil.id = factura.id;
        respuesta = await api.actualizarFactura(factura.id, cargaUtil);
      } else {
        respuesta = await api.crearFactura(cargaUtil);
      }

      if (!respuesta) {
        throw new Error("Error de conexión: No se pudo guardar la factura.");
      }

      setMensaje(factura ? "Factura actualizada correctamente" : "Factura creada correctamente");
  ...
  return (
    <div className="card tarjeta-corporativa mb-3" id="formularioFactura">
      <div className="card-body p-4">
        <div className="cabecera-formulario mb-4">
          <h5 className="fw-bold mb-1">{factura ? "Editar Factura" : "Nueva Factura"}</h5>
          <p className="text-muted small">
            {factura ? "Modifica las partidas y costes." : "Crea una nueva propuesta económica para un cliente."}
          </p>
        </div>
        ...
        <div className="acciones-formulario d-flex justify-content-end gap-2 mb-4 border-bottom pb-3">
          ...
          <button className="btn btn-primary px-5 fw-bold" onClick={alGuardarFactura} id="botonGuardarFactura">
            {factura ? "Guardar Cambios" : "Guardar Factura"}
          </button>
        </div>
        ...
        <div className="row g-3 mb-4">
          ...
          <div className="col-md-6">
            <label className="form-label fw-bold small text-muted">Estado de la Factura</label>
          ...
          <div className="col-md-6">
            <label className="form-label fw-bold small text-muted">Forma de Pago</label>
            ...
            <div className="card shadow-sm border-0 bg-light">
              <div className="card-body p-3">
                ...
                <div className="d-flex justify-content-between pt-2 border-top">
                  <span className="fw-bold text-uppercase">Total Factura:</span>
